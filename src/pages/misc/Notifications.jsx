...
const iconMap = {
  order:   { icon: ShoppingBag, bg: 'bg-emerald-50', color: 'text-emerald-600' },
  payment: { icon: CreditCard,  bg: 'bg-blue-50',    color: 'text-blue-600'    },
  edit:    { icon: Pencil,      bg: 'bg-blue-50',     color: 'text-blue-500'   },
};
    ...
    <MainLayout title="Notifications" showBack>
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-5">
          ...
          {unreadCount > 0 && (
            <button onClick={markAllRead} className="text-sm text-emerald-600 font-semibold ml-auto">
              Mark all read
            </button>
          )}
        </div>
          ...
          {FILTERS.map(f => (
            <button
              ...
              className={`px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-all ${
                filter === f ? 'bg-gray-900 text-white' : 'bg-white text-gray-500 border border-gray-100'
              }`}
            >
              {f}
            </button>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden divide-y divide-gray-50"
                  ...
                  {notifs.map(n => {
                    ...
                    return (
                      <div
                        key={n.id}
                        className={`flex items-start gap-4 px-5 py-4 ${n.unread ? 'bg-emerald-50/40' : 'hover:bg-gray-50'}`}
                      >
