...
// ... (all styles unchanged - omitted for brevity, copy from original)
// Only the back button logic changes in the main component below

const styles = `
  @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;500;600&family=DM+Sans:wght@300;400;500;600&display=swap');
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    --green-deep: #061a12; --green-dark: #0a2318; --green-mid: #0f3524;
    --green-accent: #1db87a; --green-light: #2dd881; --green-muted: #4a8c6a;
    --green-faint: #1a4a30; --white: #ffffff; --off-white: #f8f8f6;
    --border: #e8e8e4; --text-dark: #0f1f18; --text-mid: #4a5a52;
    --text-light: #8a9a92; --radius-sm: 8px; --radius-md: 12px;
    --radius-lg: 16px; --radius-xl: 20px;
  }
  html, body { height: 100%; font-family: 'DM Sans', sans-serif; background: var(--off-white); }
  .layout { display: flex; min-height: 100vh; }
  .hero { width: 46%; background: var(--green-deep); position: relative; overflow: hidden; display: flex; flex-direction: column; padding: 2.5rem 3rem; }
  .hero::before { content: ''; position: absolute; top: -80px; right: -120px; width: 600px; height: 600px; border-radius: 50%; border: 1px solid rgba(29,184,122,0.12); pointer-events: none; }
  .hero::after { content: ''; position: absolute; top: -40px; right: -180px; width: 700px; height: 700px; border-radius: 50%; border: 1px solid rgba(29,184,122,0.07); pointer-events: none; }
  .arc3 { position: absolute; top: 20px; right: -230px; width: 800px; height: 800px; border-radius: 50%; border: 1px solid rgba(29,184,122,0.05); pointer-events: none; }
  ...
  .field input[type="email"], .field input[type="password"], .field input[type="text"], .field input[type="tel"] { width: 100%; height: 46px; padding: 0 42px; border: 1px solid var(--border); border-radius: var(--radius-md); font-family: 'DM Sans', sans-serif; font-size: 13.5px; color: var(--text-dark); background: var(--white); outline: none; transition: border-color 0.2s, box-shadow 0.2s; }
  ...
  .sbar.weak { background: #e24b4a; } .sbar.fair { background: #ef9f27; } .sbar.good { background: #1db87a; } .sbar.strong { background: #0a2e25; }
  .strength-label { font-size: 11px; color: var(--text-light); }
  .match-msg { font-size: 11.5px; margin-top: 5px; min-height: 16px; }
  .match-msg.ok { color: var(--green-accent); } .match-msg.err { color: #e24b4a; }
  ...
  @media (max-width: 820px) { .layout { flex-direction: column; } .hero { width: 100%; padding: 2rem; min-height: auto; } .auth-panel { width: 100%; min-height: auto; } .hero-headline { font-size: 32px; } .reg-grid { grid-template-columns: 1fr; } }
...
const Checkbox = ({ checked, small }) => (
  <div className={`custom-checkbox${small ? " small" : ""}`} style={{ background: checked ? "var(--green-deep)" : "transparent", border: checked ? "none" : "1.5px solid #ccc" }}>
    ...
    <div>
      ...
      <div className="field">
        ...
        <div className="input-wrap">
          <svg className="input-icon" viewBox="0 0 24 24"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m2 7 10 7 10-7" /></svg>
          <input type="email" placeholder="you@restaurant.com" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSignIn()} />
        </div>
      </div>
      <div className="field">
        ...
        <div className="input-wrap">
          ...
          <input type={pwVisible ? "text" : "password"} placeholder="••••••••" autoComplete="current-password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSignIn()} />
          <button className="eye-btn" onClick={() => setPwVisible(v => !v)} aria-label="Toggle password visibility">{pwVisible ? <EyeOnIcon /> : <EyeOffIcon />}</button>
        </div>
      </div>
...
function RegisterForm({ onSuccess }) {
  ...
  const matchMsg = () => { if (!regPw2) return { text: '', cls: 'match-msg' }; if (regPw === regPw2) return { text: '✓ Passwords match', cls: 'match-msg ok' }; return { text: '✗ Passwords do not match', cls: 'match-msg err' }; };
  const match = matchMsg();

  const handleRegister = async () => {
    if (!firstName || !lastName || !businessName || !email || !regPw) { setError('Please fill in all fields.'); return; }
  ...
  return (
    <div>
      ...
      <div className="reg-grid">
        <div className="field"><label className="field-label">First Name</label><div className="input-wrap"><svg className="input-icon" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg><input type="text" placeholder="John" value={firstName} onChange={e => setFirstName(e.target.value)} /></div></div>
        <div className="field"><label className="field-label">Last Name</label><div className="input-wrap"><svg className="input-icon" viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg><input type="text" placeholder="Doe" value={lastName} onChange={e => setLastName(e.target.value)} /></div></div>
      </div>
      <div className="field"><label className="field-label">Business Type</label><div className="biz-toggle"><button type="button" className={`biz-option${businessType === 'restaurant' ? ' selected' : ''}`} onClick={() => setBusinessType('restaurant')}><svg viewBox="0 0 24 24"><path d="M3 2l1.5 9H19.5L21 2z" /><path d="M3 11v1a9 9 0 0 0 18 0v-1" /></svg>Restaurant</button><button type="button" className={`biz-option${businessType === 'shop' ? ' selected' : ''}`} onClick={() => setBusinessType('shop')}><svg viewBox="0 0 24 24"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z" /><line x1="3" y1="6" x2="21" y2="6" /><path d="M16 10a4 4 0 0 1-8 0" /></svg>Shop</button></div></div>
      <div className="field"><label className="field-label">{businessType === 'restaurant' ? 'Restaurant Name' : 'Shop Name'}</label><div className="input-wrap"><svg className="input-icon" viewBox="0 0 24 24"><path d="M3 2l1.5 9H19.5L21 2z" /><path d="M3 11v1a9 9 0 0 0 18 0v-1" /></svg><input type="text" placeholder={businessType === 'restaurant' ? 'The Grand Bistro' : 'My Shop'} value={businessName} onChange={e => setBusinessName(e.target.value)} /></div></div>
      <div className="field"><label className="field-label">Email Address</label><div className="input-wrap"><svg className="input-icon" viewBox="0 0 24 24"><rect x="2" y="4" width="20" height="16" rx="2" /><path d="m2 7 10 7 10-7" /></svg><input type="email" placeholder="you@restaurant.com" value={email} onChange={e => setEmail(e.target.value)} /></div></div>
      <div className="field"><label className="field-label">Phone Number</label><div className="input-wrap"><svg className="input-icon" viewBox="0 0 24 24" style={{ width: 15, height: 15 }}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.63 3.36 2 2 0 0 1 3.6 1.18h3a2 2 0 0 1 2 1.72c.13.96.36 1.9.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.96a16 16 0 0 0 6.13 6.13l.96-.96a2 2 0 0 1 2.11-.45c.91.34 1.85.57 2.81.7A2 2 0 0 1 22 16.92z" /></svg><input type="tel" placeholder="+27 82 000 0000" value={phone} onChange={e => setPhone(e.target.value)} /></div></div>
      <div className="field-divider"><span>Security</span></div>
      <div className="field"><label className="field-label">Password</label><div className="input-wrap"><svg className="input-icon" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg><input type={regPwVisible ? 'text' : 'password'} placeholder="Create a password" value={regPw} onChange={e => setRegPw(e.target.value)} /><button className="eye-btn" onClick={() => setRegPwVisible(v => !v)} aria-label="Toggle">{regPwVisible ? <EyeOnIcon /> : <EyeOffIcon />}</button></div><div className="pw-strength"><div className="strength-bars">{[0, 1, 2, 3].map(i => <div key={i} className={strengthClass(i, strength.score)} />)}</div><span className="strength-label" style={{ color: regPw ? strength.color : undefined }}>{strength.label}</span></div></div>
      <div className="field"><label className="field-label">Confirm Password</label><div className="input-wrap"><svg className="input-icon" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg><input type={regPw2Visible ? 'text' : 'password'} placeholder="Repeat your password" value={regPw2} onChange={e => setRegPw2(e.target.value)} /><button className="eye-btn" onClick={() => setRegPw2Visible(v => !v)} aria-label="Toggle">{regPw2Visible ? <EyeOnIcon /> : <EyeOffIcon />}</button></div><p className={match.cls}>{match.text}</p></div>
      <label className="terms-label" onClick={() => setTermsChecked(v => !v)}><Checkbox checked={termsChecked} small />I agree to the{' '}<a href="#" className="forgot-link" style={{ fontSize: 13 }}>Terms of Service</a>{' '}and{' '}<a href="#" className="forgot-link" style={{ fontSize: 13 }}>Privacy Policy</a></label>
      <button className="btn-signin" style={{ marginTop: '1rem' }} onClick={handleRegister} disabled={loading}>{loading ? 'Creating account...' : 'Create Account'}{!loading && <svg viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7" /></svg>}</button>
    </div>
  );
}
      ...
      <div className="layout">
        <div className="hero">
          ...
          <div className="features">
            <div className="feature"><div className="feature-icon-wrap"><svg viewBox="0 0 24 24"><rect x="2" y="5" width="20" height="14" rx="2" /><path d="M2 10h20" /></svg></div><span className="feature-label">Track Credit</span></div>
            <div className="feature"><div className="feature-icon-wrap"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg></div><span className="feature-label">Manage Debtors</span></div>
            <div className="feature"><div className="feature-icon-wrap"><svg viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg></div><span className="feature-label">Secure &amp; Reliable</span></div>
          </div>
          <div className="hero-footer">
            <span>© 2025 Navoq. All rights reserved.</span>
            <div className="footer-links"><a href="#">Terms</a><span className="footer-sep">|</span><a href="#">Privacy</a></div>
          </div>
        </div>
        ...
        <div className="auth-panel">
          ...
          <div className="card">
            <div className="card-avatar"><img src={NavoqLogo} alt="Navoq" style={{ width: 44, height: 44, objectFit: 'contain', borderRadius: '50%' }} /></div>
          ...
          <div className="security-note">
            ...
            <div className="security-text"><p className="security-title">Your data is protected</p><p className="security-desc">We use industry-standard security<br />to keep your information safe.</p></div>
          </div>
        </div>
      </div>
    </>
